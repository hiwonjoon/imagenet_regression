...
class Net:
    def __init__(self):
        ...
        net['fc8'] = DenseLayer(net['drop7'], num_units=1860) #, nonlinearity=None)
        net_fv = net['fc8']


        # Make a Train & a Test Function
        image = T.ftensor4('input')
        image_shared = theano.shared( np.zeros( (BATCH_SIZE, 3, 224, 224 ), dtype = np.float32 ))
        target_prob = T.fmatrix('target_prob')
        target_prob_shared = theano.shared( np.zeros( (BATCH_SIZE, 1860), dtype = np.float32) )

        # Train function
        prob  = lasagne.layers.get_output( net_fv, image )

        loss = lasagne.objectives.squared_error(prob,target_prob)
        loss = lasagne.objectives.aggregate(loss,mode='mean')

        l2_penalty = regularize_layer_params_weighted({net['fc8']: REGRESSION_LAMBDA}, l2)
        ...
        # Test function
        prob_determ  = lasagne.layers.get_output( net_fv, image, deterministic=True )
